...
function CloutApp.create(content, env)
  ...
  local C = {
    ...
    card = Color3.fromRGB(18, 18, 18), -- Dark Gray
    indigo = Color3.fromRGB(37, 244, 238), -- TikTok Cyan
    green = Color3.fromRGB(37, 244, 238), -- Cyan
    amber = Color3.fromRGB(254, 44, 85), -- TikTok Pink/Red
    red = Color3.fromRGB(254, 44, 85), -- Pink/Red
    pink = Color3.fromRGB(254, 44, 85), -- Pink/Red
    cyan = Color3.fromRGB(37, 244, 238), -- Cyan
  ...
  local scroll = Instance.new("ScrollingFrame")
    ...
    profileImage.BackgroundColor3 = C.card
    ...
    local lockedContentType = nil
    pcall(function()
      local GetLockedContentType = Remotes:FindFirstChild("GetLockedContentType")
      if GetLockedContentType then
        lockedContentType = GetLockedContentType:InvokeServer()
      end
    end)
    ...
    for i, ct in ipairs(contentTypes) do
      ...
      local lockLabel = makeLabel(lockOverlay, "Locked from prestige", 10, Color3.fromRGB(180, 80, 255), fontBold, UDim2.new(1, 0, 0, 14), UDim2.new(0, 0, 0.5, -7))
      lockLabel.TextXAlignment = Enum.TextXAlignment.Center
      lockLabel.TextYAlignment = Enum.TextYAlignment.Center
      lockLabel.ZIndex = 6
      lockLabel.Visible = false
  
      table.insert(typeCards, {card=card, stroke=stroke, check=check, ct=ct, lockOverlay=lockOverlay, lockLabel=lockLabel})
      ...
      hit.MouseButton1Click:Connect(function()
        -- Check if content type is locked from previous prestige
        if lockedContentType and lockedContentType == ct.name then
          showToast("Content type locked from previous prestige!", "Cannot change after prestige")
          return
        end
    ...
    local viralCard = makeFrame(sec3, Color3.fromRGB(20, 10, 35), UDim2.new(1, -32, 0, 70), UDim2.new(0, 16, 0, 390), 0)
    makeCorner(viralCard, 12)
    makeStroke(viralCard, Color3.fromRGB(255, 215, 0), 1.5, 0)

    local viralTitle = makeLabel(viralCard, "🔥 GOING VIRAL", 12, Color3.fromRGB(255, 215, 0), fontBold, UDim2.new(1, -16, 0, 10), UDim2.new(0, 8, 0, 0))
    viralTitle.TextXAlignment = Enum.TextXAlignment.Left

    local viralProgBg = makeFrame(viralCard, Color3.fromRGB(30, 20, 50), UDim2.new(1, -16, 0, 12), UDim2.new(0, 8, 0, 28), 0)
    makeCorner(viralProgBg, 6)
    local viralProgFill = makeFrame(viralProgBg, Color3.fromRGB(255, 215, 0), UDim2.new(0, 0, 1, 0), UDim2.new(0, 0, 0, 0), 0)
    makeCorner(viralProgFill, 6)

    local viralText = makeLabel(viralCard, "0 / 50M views", 11, C.muted, fontRegular, UDim2.new(1, -16, 0, 44), UDim2.new(0, 8, 0, 0))
    viralText.TextXAlignment = Enum.TextXAlignment.Left

    local hasGoneViral = false
    Remotes:WaitForChild("GoingViral").OnClientEvent:Connect(function(playerName, totalViews)
      if playerName == player.Name then
        hasGoneViral = true
        viralTitle.Text = "🎉 YOU WENT VIRAL!"
        viralTitle.TextColor3 = Color3.fromRGB(100, 255, 100)
        viralText.Text = "You hit 50M views! +50K coins!"
        viralText.TextColor3 = Color3.fromRGB(100, 255, 100)
        viralProgFill.BackgroundColor3 = Color3.fromRGB(100, 255, 100)
        viralProgFill.Size = UDim2.new(1, 0, 1, 0)
        makeStroke(viralCard, Color3.fromRGB(100, 255, 100), 2, 0)
      end
    end)

    local function updateViralMeter(totalViews)
      if hasGoneViral then return end
      local pct = math.clamp(totalViews / VIRAL_THRESHOLD, 0, 1)
      local pctDisplay = math.floor(pct * 100)
      TweenService:Create(viralProgFill, TweenInfo.new(0.5), {
        Size = UDim2.new(pct, 0, 1, 0)
      })
      viralText.Text = fmt(totalViews) .. " / 50M views (" .. pctDisplay .. "%)"
      if pct >= 1 then
        viralTitle.Text = "🔥 VIRAL — FIRE THE EVENT!"
        viralTitle.TextColor3 = Color3.fromRGB(255, 215, 0)
        viralProgFill.BackgroundColor3 = Color3.fromRGB(255, 215, 0)
        if not hasGoneViral then
          hasGoneViral = true
          pcall(function()
            Remotes:WaitForChild("GoingViral"):FireServer()
          end)
        end
      end
    end
    ...
    confirmBtn.MouseButton1Click:Connect(function()
      ...
      local setContentRemote = Remotes:FindFirstChild("SetContentType")
    ...
    local function processTap(autoMiss)
      ...
      local postRemote = Remotes:FindFirstChild("PostContent")
      if postRemote then
    ...
    if leaderstats then
      ...
      if v then
        vViews.Text = fmt(v.Value)
        updateViralMeter(v.Value)
      end
      local f = leaderstats:WaitForChild("Followers", 2)
      if f then 
        vFoll.Text = fmt(f.Value) 
        followersLabel.Text = fmt(f.Value) .. " Followers"
      end
    end
    ...
    local function updateLocks(folls)
           for _, t in ipairs(typeCards) do
                   if t.lockOverlay then
                           local isLockedByFollowers = folls < (t.ct.req or 0)
                           local isLockedByPrestige = lockedContentType and lockedContentType == t.ct.name
                           t.lockOverlay.Visible = isLockedByFollowers or isLockedByPrestige
                           if isLockedByPrestige then
                               t.lockOverlay.BackgroundTransparency = 0.3
                           else
                               t.lockOverlay.BackgroundTransparency = 0.5
                           end
                           -- Show prestige lock label if locked by prestige
                           if t.lockLabel then
                               t.lockLabel.Visible = isLockedByPrestige
                           end
                   end
           end
    end
    ...
    FollowersUpdated.OnClientEvent:Connect(function(totalFollowers)        
      ...
      followersLabel.Text = fmt(totalFollowers) .. " Followers"
      updateLocks(totalFollowers)
    end)

    local PrestigeResult = Remotes:WaitForChild("PrestigeResult")
    PrestigeResult.OnClientEvent:Connect(function()
      vFoll.Text = "0"
      followersLabel.Text = "0 Followers"
      vCoins.Text = "0"
      vViews.Text = "0"
      updateLocks(0)
      -- Fetch new locked content type after prestige
      pcall(function()
        local GetLockedContentType = Remotes:FindFirstChild("GetLockedContentType")
        if GetLockedContentType then
          lockedContentType = GetLockedContentType:InvokeServer()
