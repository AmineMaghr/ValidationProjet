...
import com.example.app.controllers.ChatbotPanelController;
...
public class PersonnageController extends BaseController {
    ...
    private void loadPersonnageDetails(Personnage personnage) {
        ...
        if (personnage.getPortraitImage() != null && personnage.getPortraitImage().length > 0) {
            try {
                ...
                loadGeneratedPortrait(personnage);
            }
        } else {
            loadGeneratedPortrait(personnage);
        }

        updateChatbotCharacter(personnage);
    }

    private void loadGeneratedPortrait(Personnage personnage) {
        PortraitGenerationService.loadPortraitToImageView(personnage.getNom(), portraitView);
    }
        ...
        try {
            if (tabPane != null) {
                Tab chatTab = tabPane.getTabs().stream()
                    .filter(tab -> tab.getText() != null && tab.getText().contains("Chat"))
                    .findFirst()
                    .orElse(null);
    ...
    private void updateChatbotCharacter(Personnage personnage) {
        if (chatbotController != null && personnage != null) {
            chatbotController.setCharacter(
                personnage.getNom(),
                personnage.getDescription() != null ? personnage.getDescription() : "A mysterious character"
            );
        }
    }
    ...
    private void exportCharacter() {
        if (currentPersonnage == null) {
            showAlert("Erreur", "Sélectionnez un personnage à exporter");
            return;
        }

        String jsonData = CharacterAPIService.exportCharacterAsJSON(currentPersonnage);
        System.out.println("Character exported as JSON:\n" + jsonData);
        showAlert("Succès", "Personnage exporté en JSON !\nDonnées visibles dans la console");
    }

    @FXML
    private void notifyCharacterCreation() {
        if (currentPersonnage != null && currentPersonnage.getId() > 0) {
            EmailNotificationService.notifyCharacterCreated(
                "player@example.com",
                currentPersonnage.getNom()
            );
        }
    }
}
