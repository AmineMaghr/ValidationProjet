...
public class AdvancedPreferenceService implements IService<AdvancedPreference> {  // ← IService (S majuscule)
    ...
    private void validateAdvancedPreference(AdvancedPreference p) {
        if (p == null) {
            throw new IllegalArgumentException("Les préférences ne peuvent pas être nulles.");
        }
        if (p.getFreeDescription() == null || p.getFreeDescription().trim().isEmpty()) {
            throw new IllegalArgumentException("La description libre est requise.");
        }
        if (p.getFreeDescription().trim().length() < 10) {
            throw new IllegalArgumentException("La description libre doit faire au moins 10 caractères.");
        }
        if (p.getFavoriteGenre() == null || p.getFavoriteGenre().trim().isEmpty()) {
            throw new IllegalArgumentException("Veuillez sélectionner un genre favori.");
        }
        if (p.getFavoriteThemes() == null || p.getFavoriteThemes().trim().isEmpty()) {
            throw new IllegalArgumentException("Veuillez sélectionner au moins un thème favori.");
        }
        if (p.getCustomTags() == null || p.getCustomTags().trim().isEmpty()) {
            throw new IllegalArgumentException("Les tags personnalisés sont requis.");
        }
        if (!p.getCustomTags().trim().startsWith("#")) {
            throw new IllegalArgumentException("Les tags personnalisés doivent commencer par un #.");
        }
    }

    @Override
    public void add(AdvancedPreference p) throws SQLException {
        validateAdvancedPreference(p);
